# References

The following sources identified in the GRC102 Week 2 lab assignment brief as suggested references were used:

1. **GRC102 Course Material**: Information Security Governance, Module 2: Developing Security Policies and Procedures.
2. **ISO/IEC 27001:2022**: Information security management systems requirements.
3. **ISO/IEC 27002**: Information security controls guidance.
4. **NIST Cybersecurity Framework (CSF) 2.0**: Cybersecurity risk management framework.
5. **AICPA SOC 2 Trust Services Criteria**: Criteria covering security and related trust services.

# References

## ISO

International Organization for Standardization.

[ISO/IEC 27001:2022 — Information security management systems — Requirements](https://www.iso.org/standard/27001.html)[cite: 1]

## NIST

National Institute of Standards and Technology.

[The NIST Cybersecurity Framework (CSF) 2.0](https://www.nist.gov/cyberframework)[cite: 1]

Pascoe, C., Quinn, S., & Scarfone, K. (2024).
The NIST Cybersecurity Framework (CSF) 2.0.
NIST Cybersecurity White Paper 29.[cite: 1]

## AICPA

AICPA & CIMA.

[2017 Trust Services Criteria with Revised Points of Focus — 2022](https://www.aicpa-cima.com/resources/download/2017-trust-services-criteria-with-revised-points-of-focus-2022)[cite: 1]

## Course Material

International Cybersecurity and Digital Forensics Academy.

GRC102 – Information Security Governance, Week 2:
Developing Security Policies and Procedures[cite: 1].

**Kindly note that This portfolio does not claim that NexusTech is ISO 27001 certified or SOC 2 compliant. The evidence demonstrates how policy structure and governance activities can be designed with those frameworks in mind**

## Academic Note

The assignment brief permits AI assistance for brainstorming, language, and structure while placing responsibility for verification and understanding on the student. So this is me admitting to the use of AI for brainstorming and structure of this assignment
