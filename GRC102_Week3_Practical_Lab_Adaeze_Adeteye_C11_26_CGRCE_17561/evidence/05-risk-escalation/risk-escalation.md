# Evidence Bundle 5 — Cyber-Risk Escalation

## 1. Major Cyber-Risk Escalation Workflow

```text
Risk / Incident / Control Issue Identified
                    │
                    ▼
          Operational Triage
                    │
                    ▼
       Assess Impact + Likelihood
                    │
                    ▼
     Record Risk / Incident Evidence
                    │
        ┌───────────┼───────────┐
        ▼           ▼           ▼
      Level 1     Level 2     Level 3
    Operational  Executive   Material/Board
        │           │           │
        ▼           ▼           ▼
  Operational   Executive    CEO / Board
   treatment     decision     decision
        │           │           │
        └───────────┼───────────┘
                    ▼
          Treatment / Decision
                    │
                    ▼
       Verify Control / Action
                    │
                    ▼
          Residual Risk Review
                    │
                    ▼
       Close / Monitor / Re-escalate
```

## 2. Escalation Thresholds

| Level | Trigger | Decision Authority | Notification | Target |
|---|---|---|---|---|
| Level 1 – Operational | Contained issue, low/moderate impact, within operational authority | Operational owner/management | CISO/security function | Same business day or agreed SLA |
| Level 2 – Executive | Significant disruption, cross-business impact, major control failure, material resource need, or beyond operational authority | CEO / Executive Security Council | CISO, CRO, relevant executives, Legal as needed | Prompt executive review |
| Level 3 – Material / Board | Material enterprise exposure, major incident, significant regulatory/contractual implications, substantial interruption, or beyond executive authority | CEO with Board oversight/decision where applicable | Board, CEO, CISO, CRO, Legal and affected executives | Immediate escalation appropriate to event |

These are governance thresholds, not replacements for technical incident-severity classifications. Technical severity feeds the governance assessment alongside business, legal, financial and enterprise-risk considerations.

## 3. Escalation Criteria

Escalate when:
- delegated authority is exceeded;
- multiple business units are affected;
- customer/service impact becomes material;
- significant unplanned investment is required;
- regulatory or contractual obligations may apply;
- a major control failure creates significant residual risk;
- treatment cannot be completed within the agreed period;
- key owners disagree materially;
- risk may exceed approved appetite/tolerance;
- enterprise reputation, continuity or financial exposure could be materially affected.

## 4. Decision Trail

Each Level 2/3 decision should record:

1. Decision ID
2. Date/time
3. Risk/incident reference
4. Event description
5. Business impact
6. Security impact
7. Legal/regulatory considerations
8. Risk assessment
9. Options considered
10. Recommendation
11. Decision
12. Decision authority
13. Consulted stakeholders
14. Action owners
15. Due dates
16. Residual risk
17. Review date
18. Closure evidence

## 5. Operating Rule

Escalation does not mean abandoning operational ownership. IT continues containment/remediation while governance authority moves to the appropriate decision-maker.

For a material issue:
- IT continues technical containment;
- CISO assesses security impact;
- CRO/Risk assesses enterprise exposure;
- Legal assesses relevant obligations;
- CEO/ESC makes the executive decision;
- Board receives or decides the matter when the materiality threshold is met.
